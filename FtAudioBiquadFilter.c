...
void FtAudioBiquadFilterFree(FtAudioBiquadFilter * filter)
{
	free(filter);
}

void FtAudioBiquadFilterFlush(FtAudioBiquadFilter* filter)
{
	float zero = 0.0;
	vDSP_vfill(&zero, filter->x, 1, 2);
	vDSP_vfill(&zero, filter->y, 1, 2);
...
void FtAudioBiquadFilterProcess(FtAudioBiquadFilter* filter, float*	outBuffer, const float* inBuffer, unsigned n_samples)
{
	unsigned buffer_idx;
...
void FtAudioBiquadFilterUpdateKernel(FtAudioBiquadFilter* filter, const float* bCoeff, const float* aCoeff)
{
	memcpy(filter->b, bCoeff, 3);
	memcpy(filter->a, aCoeff, 3);
}